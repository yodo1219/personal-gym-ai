...
function toClient(row: any): Client {
  return {
    ...
    messageHour: row.message_hour ?? 1,
    messageFrequency: row.message_frequency ?? "daily",
    messageEnabled: row.message_enabled ?? true,
...
function fromClient(c: Client): any {
  return {
    ...
    message_hour: (c as any).messageHour ?? 1,
    message_frequency: (c as any).messageFrequency ?? "daily",
    message_enabled: (c as any).messageEnabled ?? true,
